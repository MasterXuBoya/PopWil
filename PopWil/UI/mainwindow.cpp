...
void MainWindow::slotFuction(){
    ...
    //sCurrent=sin(2*PI*msCount/1000)+0.1*(random(1000)*1.0/500-1);//for Test
    //double test_filter=sCurrent;
    sCurrent=sAvgFilter->filter(sCurrent);//均值滤波
    //sCurrent=sHMAFilter->filter(sCurrent);//赫尔滑动滤波
    ...
    //vCurrent=vHMAFilter->filter(vCurrent);//赫尔滑动滤波
    ...
    //aCurrent=aHMAFilter->filter(aCurrent);//赫尔滑动滤波
...
double MainWindow::getPosition(int direct){
    ...
    if(ssiEnable==false){//ENC_7480相对式位移采集卡
        ...
        double x_position=readSSIData(0,hPDC1)/1000;
        position=x_position*g_dis_k+g_dis_bias;
    }
    return position-160;
}
